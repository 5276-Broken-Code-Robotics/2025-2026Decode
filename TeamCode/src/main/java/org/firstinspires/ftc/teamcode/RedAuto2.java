...
public class RedAuto2 extends OpMode {
    ...
    private final Pose scorePose = new Pose(96, 96, Math.toRadians(90)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose pickup1Pose = new Pose(125, 84, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose pickup2Pose = new Pose(125, 60, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose pickup3Pose = new Pose(125, 36, Math.toRadians(0)); // Lowest (Third Set) of Artifacts from the Spike Mark.
    ...
    private final Pose leverPose = new Pose(128, 76, Math.toRadians(270)); // Lowest (Third Set) of Artifacts from the Spike Mark.
    private final Pose leverRotatePose = new Pose(111, 76, Math.toRadians(270)); // Lowest (Third Set) of Artifacts from the Spike Mark.

    private PathChain rotate1, grabPickup1, rotateForLever, hitLever, shootPickup1, rotate2, grabPickup2, shootPickup2, rotate3, grabPickup3, shootPickup3,finalPosition;
    ...
    public void buildPaths(){
        ...
        hitLever= follower.pathBuilder()
                .addPath(new BezierLine(leverRotatePose, leverPose))
                .setLinearHeadingInterpolation(leverRotatePose.getHeading(), leverPose.getHeading())
                .build();
        rotateForLever= follower.pathBuilder()
                .addPath(new BezierLine(pickup1Pose, leverRotatePose))
                .setLinearHeadingInterpolation(pickup1Pose.getHeading(), leverRotatePose.getHeading())
                .build();
    }
        ...
        switch (pathState) {
            ...
            case 3:
                ...
                if (!follower.isBusy()) {
                    ...
                    follower.followPath(rotateForLever, true);
                    setPathState(4);
                }
                break;
            case 4:
